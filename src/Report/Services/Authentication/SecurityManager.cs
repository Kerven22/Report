...
namespace Report.Services.Authentication
{
    internal class SecurityManager : ISecurityManager
    {
        ...
        public bool ComparePassword(string paswword, string salt, string hash)
        {
            var newHash = ComputeSha(paswword, salt); 
        ...
        public (string Salt, string Hash) GeneratePasswordPart(string password)
        {
        ...
        {
            var buffer = Encoding.UTF8.GetBytes(plainText).Concat(Encoding.UTF8.GetBytes(salt)).ToArray(); 
            lock(sHA256)
            {
